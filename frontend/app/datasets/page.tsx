...
import { DatasetCreateDialog } from "@/components/dataset/dataset-create-dialog";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 rounded-lg bg-zinc-100 p-1 w-fit">
        {TABS.map((tab) => {
          const count = datasets.filter((d) => d.status === tab.key).length;
          return (
            <button
              key={tab.key}
              type="button"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-200 bg-zinc-50 text-left text-xs font-medium text-zinc-500 uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 hidden lg:table-cell">记录数</th>
                  <th className="px-4 py-3 hidden sm:table-cell">负责人</th>
                  <th className="px-4 py-3 hidden xl:table-cell">存储位置</th>
                  <th className="px-4 py-3">状态</th>
                  <th className="px-4 py-3 hidden md:table-cell w-36">更新时间</th>
                ...
                {paged.length === 0 ? (
                  <tr>
                    ...
                    >
                      {searchQuery.trim()
                        ? "未找到匹配的数据集"
                        : `暂无${TABS.find((t) => t.key === activeTab)?.label ?? ""}状态的数据集记录`}
                    </td>
                  </tr>
                ) : (
                  paged.map((dataset, idx) => (
                    <tr
                      key={dataset.id}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-zinc-500 hidden xl:table-cell font-mono text-xs max-w-[220px] truncate">
                        {dataset.locationPath}
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={datasetStatusVariant(dataset.status)}>
                          {dataset.statusLabel}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-zinc-500 hidden md:table-cell text-xs">
                        {dataset.updatedAtLabel}
                      </td>
